...
class Repository:
    ...
    def AddUser(self, User):
        if (User.password is not None):
            ...
            query = """INSERT INTO USERS (first_name,last_name,user_national_code,user_email,user_passwordhash,user_salt,user_registerdate,balance) VALUES ('%s','%s','%s','%s','%s','%s','%s','%s');""" \
                    % (User.first_name, User.last_name, User.national_code, User.email, finalpasswordhash, User.salt,
                       User.registrationdate, User.balance)
    ...
    def CheckUser(self, username, password):
        ...
        if (salt is None):
            ...
            print("Run Checkuser")
            finalpasswordhash = passwordhash.hexdigest()
            checkquery = """SELECT user_email FROM users U WHERE U.user_email = '%s' AND U.user_passwordhash = '%s' """ % (
            username, finalpasswordhash)
            print(checkquery)
    ...
    def AddService(self, Service):
        if (Service.service_name is not None & Service.resource_id is not None & Service.stock >= 1):
            query = """INSERT INTO services(service_name,resource_id_fk,stock) VALUES('%s','%s','%s')""" % (
            Service.service_name, Service.resource_id, Service.stock)
            print(query)
            try:
                self.Cursor.execute(query)
                self.DBConnection.commit()
    ...
    def AddUser_Service(self, User, Service, Packet_Number):
        if (
                Service.service_id is not None & Service.service_name is not None & Service.resource_id is not None & Service.stock >= 1 & User.user_id is not None):
